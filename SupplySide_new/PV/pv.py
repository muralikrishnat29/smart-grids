...
class pv:
    ...
    # Lat - Latitude degree
    # Day_of_Year
    # Shorizontal - solar irradiance W/m^2
    ...
    # INPUT:
    # T, Day_of_Year, Shorizontal, Lat called from .get_current_parameters()
    # OUTPUT:
    ...
    def energy_cal_cur(self):
        ...
        p_soalr_irradiance = (
            Shorizontal * abs(sin(radians(p_alpha + p_beta))))/abs(sin(p_alpha))
        #p_PR=((((-1)**(T-25-273.15))* ((0.5)**abs(T-25-273.15)))* 100)+14
        p_PR = ((-0.5)*(T-273.15-25))+14
        #print("ppr",p_PR)
        #print("horiz",Shorizontal)
        p_power_coeff = (Emax / Area)
        E = Area*p_power_coeff*p_PR*p_soalr_irradiance*.001
        E = numpy.around(E, 3)
    ...
    # Calculate the current power generated
    # INPUT:
    # T, Day_of_Year, Shorizontal, Lat called from .get_forecast_parameters()
    # OUTPUT:
